...
module.exports = function (app) {
  ...
  app.post("/endSession", function (req, res, next) {
    ...
    console.log("removed access token from server-side session");
  ...
  app.post("/getAccessToken", function (req, res, next) {
    ...
    var options = {
      ...
      form: {
        ...
        resourceServer: "Identity Provider",
      },
      headers: {
    ...
    request(options, function (error, response, body) {
      ...
      pointblueJwtVerifier
        .verifyAccessToken(obj.id_token, process.env.CLIENT_ID)
        .then((jwt) => {
          response_to_browser.id_token_decoded = jwt.claims;
          console.log(jwt.claims);

          pointblueJwtVerifier
            .verifyAccessToken(obj.access_token, process.env.CLIENT_ID)
            .then((jwt) => {
            ...
            .catch((err) => {
              console.log(
                "something went wrong with the access_token validation"
              );
              console.log(err);
            });
        })
  ...
  app.post("/getData", function (req, res, next) {
    ...
    if (
      process.env.hasOwnProperty("GATEWAY_URI") &&
      process.env.GATEWAY_URI != ""
    ) {
